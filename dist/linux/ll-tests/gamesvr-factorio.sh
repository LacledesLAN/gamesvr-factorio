...
# Generate an empty map for the Factorio server to use
echo Generating an empty Factorio map...
{ /app/bin/x64/factorio --create app/saves/save.zip; } &> /dev/null;
if [ $? -ne 0 ]; then
    echo "was unable to create empty map file, needed for testing!";
    exit 2;
fi;
echo ...done.
...
#####################################################################################################
### CONFIG VARS #####################################################################################
declare LLTEST_CMD="/app/bin/x64/factorio --start-server app/saves/save.zip;";
...
#####################################################################################################
### TESTS ###########################################################################################
should_have 'Operating system: Linux' 'operating system reported as expected';
should_have 'Read data path: /app/data' 'data path is as expected';
should_have 'Binaries path: /app/bin' 'binaries path is as expected';
should_have 'Running in headless mode' 'Server reported as headless';
should_have 'Factorio initialised' 'Server reported as started';
should_lack 'MultiplayerManager failed' 'Server did not report multiplayer issue';
should_lack 'Goodbye' 'Server did not unexpectedly quit';

should_echo 'Happiness consists in getting enough sleep. Just that, nothing more.' '<server>: Happiness consists in getting enough sleep. Just that, nothing more.';
#####################################################################################################
#####################################################################################################
